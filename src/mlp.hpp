...
inline void read_image_from_ddr(dImage_ *input, dImage_ buffer[__MAX_BUFFER_SIZE__], dType_Reg size) {
...
void fullyConnected(dImage_ *input, dType_ weights[__L1_COLS__][__L1_COLS__], dType_ *bias, dImage_ *output, dType_Reg row, dType_Reg col) {
    dType_Reg sum = 0;
    int tmp_in = 0;
    int tmp_weight = 0;
    for(int i = 0; i < row; i++) {
...
{   
    ...
    read_weight_from_ddr(l1_w, l1_w_buf, __L1_ROWS__, __L2_COLS__);
